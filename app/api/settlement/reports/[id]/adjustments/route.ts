...
export async function GET(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (scope.scope === "team") {
    const { data: owner } = await supabaseAdmin.from("users").select("team_name").eq("id", report.user_id).maybeSingle();
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const report = await fetchReport(id);
  if (!report) return NextResponse.json({ error: "리포트를 찾을 수 없습니다." }, { status: 404 });
  if (report.status === "confirmed") return NextResponse.json({ error: "확정된 월 리포트는 조정할 수 없습니다." }, { status: 400 });
  ...
  if (!recompute.ok) {
    return NextResponse.json({ error: typeof recompute.error === "string" ? recompute.error : recompute.error.message }, { status: 400 });
  }
