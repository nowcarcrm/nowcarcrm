...
import { generatePersonalReportExcel } from "@/app/(admin)/_lib/settlement/excelExporter";
...
export async function GET(req: Request, context: { params: Promise<{ userId: string }> }) {
  ...
  const { data: targetUser } = await supabaseAdmin
    .from("users")
    .select("id,name,email,rank,team_name")
    .eq("id", userId)
    .maybeSingle();
  ...
  const adjustments = (rawAdjustments ?? []) as AdjustmentRow[];

  const buffer = generatePersonalReportExcel(report, deliveries, adjustments);
  const fileName = `정산서_${report.user_name}_${month}.xlsx`;
