...
export async function POST(
  ...
  try {
    ...
    if (!canApproveByRank(requester.rank)) {
      return NextResponse.json({ error: "본부장 이상만 연차요청을 반려할 수 있습니다." }, { status: 403 });
    }

    const { id } = await params;
    if (!id?.trim()) return NextResponse.json({ error: "요청 ID가 필요합니다." }, { status: 400 });
    const body = (await req.json()) as { rejectionReason?: string };
    ...
    if (error) throw new Error(error.message);
    if (!data) {
      return NextResponse.json({ error: "대기 상태 요청만 반려할 수 있습니다." }, { status: 400 });
    }
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "반려 처리 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
